...
def complete_production(serial_no: str) -> dict:
    """
    모든 선행 필수 공정이 PASS인지 확인하고,
    생산 완료 공정을 등록한 뒤 Serial을 PASS 처리한다.
    """
    ...
    with get_connection() as connection:
        ...
        if incomplete_step is not None:
            if incomplete_step["result"] == "FAIL":
                reason = "FAIL"
            else:
                reason = "미등록"
            ...
            raise ValueError(
                ...
                f"({incomplete_step['process_name']}, {reason})"
            )
